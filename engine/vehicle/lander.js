...
/* Five recovered structural materials complete the eight physical groups.
   Several groups share one material system: a pressure material includes its
   service cells, and the two sensor materials each fix a structural pair. */
...
export class Lander {
  constructor(heightAt) {
    ...
    this.parts = RESTORATION_PARTS.map((name, index) => ({
      index, name, assembly: STRUCTURAL_ASSEMBLY_BY_PART[index],
      objects: [], wire: null, wireMaterial: null,
  ...
  /** A final wire-to-matter registration pass used only by the 5+3 tableau.
  ...
  setCompletionHighlight(progress = null) {
    ...
    /* The physical register and the 3D contour consume this same reusable
       state. Keeping the arrays here prevents the DOM instrument from
       inventing a second timing model or allocating on every frame. */
    ...
    for (const part of this.parts) {
      ...
      registration.weights[part.assembly] = Math.max(
        registration.weights[part.assembly], opacity);
      ...
      part.wireMaterial.color.setHex(part.assembly === this.structureCount - 1 ? 0xffd57a : 0xffb21c);
  ...
  dockingSurface(x, z, terrain) {
    ...
    /* The bay floor remains a physical support after the door closes. The old
       ramp-progress gate removed this surface during closing, so the hidden
       rover suspension converged toward terrain below the lander and later
       appeared to fall through the hull. Only the exterior ramp is gated. */
  ...
  update(now, active = true) {
    ...
    /* Keep the final wire-state core registered with the roof. The crown only
       begins its search motion after the fifth structural material settles. */
    this.crown.rotation.y = this.parts[7].state === 'solid'
      ? Math.sin(t * 0.095) * 0.16 : 0;
    const normalSignal = this.restorationComplete ? signalEnvelope(t - 0.90, 3.2) : 0;
