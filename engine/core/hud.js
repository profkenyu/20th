...
#fh-mission{
  position:fixed;z-index:30;top:calc(var(--frame-top) + 22px);left:24px;width:252px;
  pointer-events:none;font-family:'DM Mono',ui-monospace,monospace;text-transform:uppercase;
  color:#eceef0;border-left:1px dotted rgba(91,238,128,.76);padding:4px 9px 4px 12px;
  background:linear-gradient(90deg,rgba(5,5,6,.58),rgba(5,5,6,.14) 82%,transparent);
  text-shadow:0 1px 9px rgba(0,0,0,.90);transition:opacity .8s ease;
}
#fh-mission>span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#fh-mission .experience{margin-bottom:4px;font:500 8px/1.45 'DM Mono',ui-monospace,monospace;
  letter-spacing:.18em;color:rgba(181,186,194,.84);transition:color .8s ease,letter-spacing .8s ease}
#fh-mission .body{display:flex;align-items:center;gap:9px;min-height:17px;color:#d01a30}
#fh-mission .body-code{flex:none;--dot-size:1.35px;--dot-gap:.8px;--dot-character-gap:2.5px;
  --dot-colour:#85f2a8;--dot-off:.065;--dot-on:.88;--dot-glow:2px}
#fh-mission .body-label{min-width:0;overflow:hidden;text-overflow:ellipsis;
  font:700 9px/1.5 'Space Mono',ui-monospace,monospace;letter-spacing:.17em;color:#d01a30}
#fh-mission .objective{display:flex;align-items:center;gap:8px;margin-top:4px;
  font-size:9px;line-height:1.55;letter-spacing:.10em;color:rgba(237,239,241,.94)}
#fh-mission .objective-label{min-width:0;overflow:hidden;text-overflow:ellipsis}
#fh-mission .objective-code{flex:none;--dot-size:1.1px;--dot-gap:.7px;--dot-character-gap:2px;
  --dot-colour:#d9dde2;--dot-off:.055;--dot-on:.82;--dot-glow:1px}
#fh-mission .systems{font-size:8px;line-height:1.65;letter-spacing:.09em;color:rgba(181,186,194,.82)}
body.fh-dead #fh-mission{opacity:.32}
body.ti-observer #fh-mission{opacity:.94}
body.ti-explorer #fh-mission{opacity:1;border-left-color:rgba(91,238,128,.82)}
...
@media (max-width:760px){
  ...
  #fh-mission{width:min(200px,calc(100vw - 164px));left:14px;
    top:calc(var(--frame-top) + 14px);padding-left:9px}
  #fh-spark{width:206px}
  #fh-keys{left:14px;right:14px;bottom:calc(var(--frame-bottom) + 12px);font-size:7px;letter-spacing:.06em}
  #fh-mission .body{gap:6px}
  #fh-mission .body-code{--dot-size:1.05px;--dot-gap:.65px;--dot-character-gap:2px}
  #fh-mission .body-label{font-size:8px;letter-spacing:.12em}
  #fh-mission .objective-code{--dot-size:.9px;--dot-gap:.55px;--dot-character-gap:1.5px}
}
`;
...
export class Hud {
  ...
  constructor(rows = [], pulseField = 'recentre', { diagnostic = false } = {}) {
    ...
    mission.innerHTML = '<span class="experience">OBSERVER · AUTONOMOUS</span>'
      + '<span class="body"><span class="body-code"></span><span class="body-label">SHEAR WORLD</span></span>'
      + '<span class="objective"><span class="objective-label">LANDER SHELL</span><span class="objective-code"></span></span>'
  ...
  setMission({ body, objective, systems }) {
    ...
    if (body != null && this.missionBody !== body) {
      ...
      renderDotMatrix(this.missionEl.querySelector('.body-code'), code,
    ...
    if (objective != null && this.missionObjective !== objective) {
      ...
      if (match) renderDotMatrix(code, `${match[2]}/${match[3]}`,
