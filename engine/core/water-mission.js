...
/**
 * BODY 02 has one scientific objective: confirm water-equivalent hydrogen in
 * a real terrain lens.  Nothing here is a pickup marker.  The dark fragments,
 * absorption rings and changing local particle density are the evidence.
 */
export class WaterMission {
  constructor(heightAt, site, onConfirmed = null) {
    ...
    const fragmentMaterial = new THREE.MeshBasicMaterial({ color: 0x0a151a });
    for (let i = 0; i < 17; i++) {
      const a = hash(i * 17 + 2) * Math.PI * 2;
      const r = 0.18 + hash(i * 23 + 5) * 1.62;
      const fragment = new THREE.Mesh(fragmentGeometry, fragmentMaterial);
      fragment.position.set(Math.cos(a) * r, 0.025 + hash(i * 31) * 0.08,
        Math.sin(a) * r * 0.62);
      fragment.scale.set(0.35 + hash(i * 41) * 1.15,
        0.12 + hash(i * 43) * 0.26, 0.30 + hash(i * 47) * 0.94);
    ...
    for (let i = 0; i < 3; i++) {
      const ring = new THREE.Mesh(
        new THREE.RingGeometry(0.80 + i * 0.39, 0.815 + i * 0.39, 96),
        new THREE.MeshBasicMaterial({
          color: [0x6f9fb3, 0xa9d1d5, 0xc0152a][i],
          transparent: true, opacity: 0.025, depthWrite: false,
          blending: THREE.AdditiveBlending,
        }),
      );
      ring.rotation.x = -Math.PI / 2;
      ring.position.y = 0.012 + i * 0.005;
      ring.scale.y = 0.68;
    ...
    this.particles = new THREE.Points(particleGeometry, new THREE.PointsMaterial({
      color: 0x8ebdc8, size: 0.028, sizeAttenuation: true,
      transparent: true, opacity: 0.16, depthWrite: false,
    }));
    this.group.add(this.particles);
  }

  get active() { return this.state !== 'inactive' && this.state !== 'confirmed'; }
  get complete() { return this.state === 'confirmed'; }
  get target() { return this.active ? this.site : null; }
  get scanFocus() { return this.event ? this.site : null; }
  snapshot() {
  ...
  activate(now = performance.now()) {
    ...
    this.rings.forEach(ring => { ring.material.opacity = 0.025; });
  ...
  shouldHold(probe) {
    ...
    return Math.hypot(probe.x - this.site.x, probe.z - this.site.z)
      <= this.site.acquireRadius + 0.35;
  }
  ...
  _confirm(now) {
    ...
    this.event = null;
    this.rings.forEach((ring, i) => { ring.material.opacity = 0.12 - i * 0.018; });
  ...
  update(probe, now, worldActive = true) {
    ...
    const t = now * 0.001;
    const integration = this.event
      ? clamp01((now - this.event.t0) / this.site.scanHoldMs) : 0;
    ...
    this.rings.forEach((ring, i) => {
      ring.material.opacity = this.complete ? 0.09 - i * 0.012
        : 0.022 + integration * (0.14 - i * 0.018);
