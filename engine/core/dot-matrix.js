/*
 * A deliberately small instrument alphabet shared by the mission displays.
 * It is geometry, not a decorative webfont: every lit cell belongs to a live
 * state value and the unlit matrix remains faintly visible like early LCD/CRT
 * hardware. Korean prose and dimensional annotations never pass through it.
 */
...
export function installDotMatrixStyles() {
  ...
  style.textContent = `
    .ti-dot-matrix{display:inline-flex;align-items:center;gap:var(--dot-character-gap,3px);
      color:var(--dot-colour,#85f2a8);line-height:1}
    .ti-dot-character{display:grid;grid-template-columns:repeat(5,var(--dot-size,2px));
      grid-template-rows:repeat(7,var(--dot-size,2px));gap:var(--dot-gap,1px)}
    .ti-dot-pixel{width:var(--dot-size,2px);height:var(--dot-size,2px);
      background:currentColor;opacity:var(--dot-off,.075)}
    .ti-dot-pixel.on{opacity:var(--dot-on,.92);box-shadow:0 0 var(--dot-glow,3px) currentColor}
  `;
...
export function drawSevenSegment(ctx, text, x, y, options = {}) {
  ...
  const off = options.off ?? 'rgba(76,218,115,.07)';
  const middle = (height - thickness) * .5;
  const segmentRects = {
  ...
  for (const character of String(text ?? '')) {
    if (character === '/') {
      ctx.fillStyle = on;
      ctx.save();
      /* Canvas Y grows downward. A positive quarter turn from the vertical
         produces the conventional slash: lower-left to upper-right. */
      ctx.translate(cursor + 9, y + 4);
      ctx.rotate(Math.PI / 4);
      ctx.fillRect(0, 0, thickness, height * .55);
      ctx.restore();
      cursor += 11 + spacing;
      continue;
    }
