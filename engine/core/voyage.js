...
const CSS = `
#ti-voyage{position:fixed;z-index:30;inset:var(--frame-top) 0 var(--frame-bottom);pointer-events:none;opacity:0;transition:opacity 1.2s;color:rgba(224,228,232,.78);font:9px/1.6 'DM Mono',monospace;letter-spacing:.2em}
...
export class VoyageSequence {
  ...
  start(destination, now = performance.now()) {
    ...
    /* The rover remains physically supported by the closed hangar during
       hold and lift. Destination preparation resets this support before the
       next deployment pose is established. */
