...
export const RESTORATION_ITEMS = Object.freeze([
  Object.freeze({ at: 24,  kind: 'structure', structure: 0, sample: 'FE–NI ALLOY',
    module: 'FOUNDATION', color: 0xd88435, sign: 'METALLIC REFLECTANCE' }),
  Object.freeze({ at: 58,  kind: 'structure', structure: 1, sample: 'SILICATE CERAMIC',
    module: 'LOAD PATHS', color: 0xc9b58b, sign: 'SPECTRAL SPLIT' }),
  Object.freeze({ at: 96,  kind: 'structure', structure: 2, sample: 'CARBON COMPOSITE',
    module: 'SERVICE / PRESSURE', color: 0x8fa6ad, sign: 'LOW ALBEDO DENSITY' }),
  Object.freeze({ at: 138, kind: 'structure', structure: 3, sample: 'CONDUCTIVE LATTICE',
    module: 'TRANSFER / VISOR', color: 0xe2a94b, sign: 'METALLIC LATTICE' }),
  Object.freeze({ at: 184, kind: 'structure', structure: 4, sample: 'RARE-EARTH CERAMIC',
    module: 'SENSOR / SIGNAL', color: 0xd0b46d, sign: 'SPECULAR BANDING' }),
  Object.freeze({ at: 234, kind: 'reserve', reserve: 0, sample: 'N₂ RESERVE',
    module: 'ATMOSPHERE BASE', gauge: 'N₂', color: 0xb9c9c4, sign: 'RAMAN N₂ BAND' }),
  Object.freeze({ at: 288, kind: 'reserve', reserve: 1, sample: 'H₂O RESERVE',
    module: 'O₂ / POTABLE BASE', gauge: 'H₂O', color: 0xa9d7e2, sign: 'O–H ABSORPTION' }),
  Object.freeze({ at: 346, kind: 'reserve', reserve: 2, sample: 'ALCOHOL · C₂H₅OH',
    module: 'CHEMICAL FEEDSTOCK', gauge: 'EtOH', color: 0xffb21c, sign: 'C–O / O–H SPECTRUM' }),
]);
...
/**
 * Eight deterministic surface recoveries. The first five material systems
 * fix the lander's exterior; the remaining nitrogen, water and alcohol
 * signatures charge three finite reserve gauges without adding geometry.
 */
export class Restoration {
  ...
  _buildSites() {
    ...
    for (let index = 0; index < this.items.length; index++) {
      ...
      const cool = new THREE.Mesh(shadowGeometry, new THREE.MeshBasicMaterial({
        color: index === 6 ? 0x6a93a8 : 0x17252a, transparent: true,
        opacity: index === 6 ? 0.18 : 0.07, depthWrite: false,
      }));
  ...
  get structuralCount() { return Math.min(this.count, STRUCTURAL_MATERIAL_COUNT); }
  get structureComplete() { return this.structuralCount >= STRUCTURAL_MATERIAL_COUNT; }
  get reserveCount() { return Math.max(0, this.count - STRUCTURAL_MATERIAL_COUNT); }
  get gauges() {
    return this.items.slice(STRUCTURAL_MATERIAL_COUNT).map((item, index) => ({
      code: item.gauge, label: item.module, sample: item.sample,
      value: Number(this.gaugeValues[index] ?? 0), acquired: this.reserveCount > index,
    }));
  }
  ...
  snapshot() {
    return { count: this.count, complete: this.complete, state: this.state,
      structure: { count: this.structuralCount, total: STRUCTURAL_MATERIAL_COUNT,
        complete: this.structureComplete },
      reserves: { count: this.reserveCount, total: RESERVE_GAUGE_COUNT,
        gauges: this.gauges },
      ...
      registration: {
        ...
        registered: this.cells.reduce((sum, cell) =>
          sum + (cell.classList.contains('registered') ? 1 : 0), 0),
  ...
  _syncUi(message = '') {
    if (this.progress) this.progress.textContent =
      `SHELL ${this.structuralCount} / ${STRUCTURAL_MATERIAL_COUNT} · RESERVE ${this.reserveCount} / ${RESERVE_GAUGE_COUNT}`;
    for (let i = 0; i < this.cells.length; i++)
      this.cells[i].classList.toggle('on', i < this.structuralCount);
    this._syncGauges();
    if (this.label) this.label.textContent = message || (this.complete
      ? 'LANDER + RESERVE STATE · COMPLETE'
      : this.structureComplete
        ? `LANDER FIXED · RESERVE ${this.reserveCount} / ${RESERVE_GAUGE_COUNT}`
        : this.count ? `${this.items[this.count - 1].module} · MATERIAL FIXED`
                     : 'WIRE STATE · 5 STRUCTURES + 3 RESERVES REQUIRED');
  ...
  _syncGauges() {
    this.gaugeElements.forEach((element, index) => {
      ...
      element.classList.toggle('active', this.event?.item?.kind === 'reserve'
        && this.event.item.reserve === index);
  ...
  _buildRegistrationCells() {
    this.cells.forEach((cell, index) => {
      ...
      cell.style.setProperty('--material-colour',
    ...
    this.gaugeElements.forEach((element, index) => {
      ...
      element.style.setProperty('--material-colour',
        `#${item.color.toString(16).padStart(6, '0')}`);
    });
    this._syncGauges();
  }

  /** Mirror the lander's wire-to-matter pass in the existing evidence panel.
      The supplied arrays are owned by Lander and reused every frame; this
      method only changes five CSS scalars and text when the active material
      changes. No independent completion clock is created here. */
  setCompletionRegistration(registration = null, timelineProgress = 0) {
    ...
    const fixed = allSolid && (this.registrationReduced || registration.progress >= 0.70);
    const activeIndex = this.registrationReduced || fixed
      ? STRUCTURAL_MATERIAL_COUNT - 1
      : Math.max(0, Math.min(STRUCTURAL_MATERIAL_COUNT - 1, registration.activeIndex));
    ...
    if (fixed) {
      ...
      if (this.registrationModule) this.registrationModule.textContent =
        '관측 완료 · 외부 구조재 5종 고정 · 자원 3계통 충전';
    } else {
      if (this.registrationPhase) this.registrationPhase.textContent =
        `REGISTRATION PASS · ${String(registered).padStart(2, '0')} / 05`;
      if (this.registrationSample) this.registrationSample.textContent = item.sample;
      if (this.registrationModule) this.registrationModule.textContent =
        `${item.sign} → ${item.module}`;
    }
  }
  ...
  acquireAll(v, now = performance.now()) {
    ...
    /* '=' is a decisive PLANET 01 boundary condition. If a single-site scan
       is already integrating, retire that local event before fixing the full
       eight-channel state at one shared timestamp. */
    this.event = null;
    this.holdUntil = 0;
    ...
    this.group.visible = true;
    this.root?.classList.add('active');
    this._syncUi('5 STRUCTURES + 3 RESERVES · ACQUIRED');
    return true;
  }
  ...
  update(v, now, active = true) {
    ...
    if (this.event.all) {
      ...
      if (now - this.event.t0 >= EVENT_MS) {
        ...
        this._syncUi('LANDER FIXED · THREE RESERVES CHARGED');
      }
      return;
    }
    ...
    if (!this.event.committed && ageMs >= SCAN_MS) {
      const fixed = this.event.item.kind === 'structure'
        ? this.lander.restorePart(this.event.item.structure, now) : true;
      if (fixed) {
