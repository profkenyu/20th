...
export class Wake {
  constructor() {
    ...
    this.uTrackA = uniform(vec2(0, 0));
    this.uTrackB = uniform(vec2(0, 0));
    this.uDecay = uniform(float(1));
    this.uGain = uniform(float(0));
    this.uShift = uniform(vec2(0, 0));
    const localOf = (i) => {
    ...
    this.passStep = Fn(() => {
      ...
      const g = (d) => exp(d.mul(d).div(r.mul(r).mul(-2)));
      const stamp = max(g(length(p.sub(this.uTrackA))), g(length(p.sub(this.uTrackB))));
      const prev = this.buf.element(i);
      this.buf.element(i).assign(min(float(1), prev.mul(this.uDecay).add(stamp.mul(this.uGain))));
    })().compute(COUNT);
    this.passShiftOut = Fn(() => {
      const i = instanceIndex;
      const col = int(i.mod(uint(G))).add(int(this.uShift.x));
      const row = int(i.div(uint(G))).add(int(this.uShift.y));
      const inside = step(float(0), float(col)).mul(step(float(col), float(G - 1))).mul(step(float(0), float(row))).mul(step(float(row), float(G - 1)));
    ...
    this.sample = Fn(([lx, lz]) => {
      ...
      const i = uint(clamp(round(gz), 0, G - 1).mul(G).add(clamp(round(gx), 0, G - 1)));
      return this.buf.element(i).mul(inside);
    });
    this.sampleSmooth = Fn(([lx, lz]) => {
      const gx = clamp(lx.div(CELL).add((G - 1) * 0.5), 0, G - 1.002).toVar();
      const gz = clamp(lz.div(CELL).add((G - 1) * 0.5), 0, G - 1.002).toVar();
