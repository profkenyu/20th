...
export class ResolutionTransferFX {
  constructor(roverGroup) {
    ...
    this._dust = new THREE.Color(13143640);
    this._crimson = new THREE.Color(12588330);
    this.mode = "idle";
  ...
  beginDeparture(reason) {
    ...
    this.vortex.material.color.setHex(7827821);
    this.core.visible = false;
    this.axis.visible = true;
    this.halos.forEach((ring) => {
      ring.visible = true;
      ring.material.color.setHex(10133928);
    });
  ...
  depart(amount) {
    ...
    for (const part of PARTS) {
      ...
      const spin = (this.emergency ? 5 : 3.2) * collapse * Math.PI;
      for (let i = 0; i < cloud.count; i++) {
        ...
        const liftFade = 1 - smooth((q - 0.3) / 0.7);
        cloud.pos[o + 1] = this.center.y + (cloud.base[o + 1] - this.center.y) * scale + chassisY + this.partVertical(part, q, false) * liftFade + Math.sin(cloud.phase[i] + q * 10) * 0.025 * q;
  ...
  blackout(elapsedMs = 0) {
    ...
    this.vortex.visible = false;
    const fade = 1 - smooth((elapsedMs - 2e3) / 600);
    const shimmer = 0.82 + Math.sin(elapsedMs * 91e-4) * 0.11 + Math.sin(elapsedMs * 37e-4 + 1.4) * 0.07;
    this.axis.visible = fade > 1e-3;
    this.axis.material.opacity = 0.065 * fade * shimmer;
    const breathe = 0.985 + Math.sin(elapsedMs * 43e-4) * 0.018;
  ...
  prepareArrival() {
    ...
    this.group.visible = true;
    this.vortex.visible = true;
    this.vortex.material.color.setHex(12024648);
    this.core.visible = false;
    this.axis.visible = true;
    this.halos.forEach((ring) => {
      ring.visible = true;
      ring.material.color.setHex(13143640);
    });
    this.core.material.opacity = 0;
  ...
  arrive(amount) {
    ...
    for (const part of PARTS) {
      ...
      for (let i = 0; i < cloud.count; i++) {
        ...
        const dx = cloud.base[o] - this.center.x;
        const dz = cloud.base[o + 2] - this.center.z;
        const a = spin * (0.65 + hash(i + PARTS.indexOf(part) * 911) * 0.7);
        const c = Math.cos(a), s = Math.sin(a);
        cloud.pos[o] = this.center.x + (dx * c - dz * s) * scale;
        cloud.pos[o + 1] = this.center.y + (cloud.base[o + 1] - this.center.y) * scale + chassisY + this.partVertical(part, q, true) + Math.sin(cloud.phase[i] + (1 - q) * 9) * 0.022 * inv;
        cloud.pos[o + 2] = this.center.z + (dx * s + dz * c) * scale;
      }
      cloud.geo.attributes.position.needsUpdate = true;
  ...
  updateVortex(amount, reverse) {
    const p = clamp(amount), t = performance.now() * 1e-3;
    for (let i = 0; i < this.vortexCount; i++) {
      ...
      const a = hash(i + 219) * Math.PI * 2 + t * (0.28 + q * 0.3) + (reverse ? -1 : 1) * p * (4 + q * 7);
  ...
  updateField(amount, arrival) {
    ...
    const axisEnvelope = arrival ? 1 - smooth(p / 0.72) : smooth((p - 0.08) / 0.7) * (1 - smooth((p - 0.9) / 0.1));
    const shimmer = 0.82 + Math.sin(now * 47e-4) * 0.11 + Math.sin(now * 19e-4 + 1.7) * 0.07;
    this.axis.material.opacity = axisEnvelope * (arrival ? 0.075 : 0.09) * shimmer;
    const fieldBreath = 0.97 + Math.sin(p * Math.PI) * 0.035 + Math.sin(now * 26e-4) * 0.012;
    ...
    for (let i = 0; i < this.halos.length; i++) {
      ...
      ring.position.set(this.center.x, this.center.y + 0.035 + i * 8e-3, this.center.z);
      ring.scale.setScalar(radius);
      ring.material.opacity = Math.sin(Math.PI * q) * (arrival ? 0.07 : 0.055) * (1 - i * 0.14);
    }
  }
  setMeshVisibility(amount, arrival) {
    ...
    const windows = arrival ? ARRIVE : DEPART;
    for (const mesh of this.meshes) {
      if (this.originalVisibility.get(mesh) === false) {
        mesh.visible = false;
        continue;
      }
      const q = local(p, windows[this.meshPart.get(mesh) ?? "body"]);
      mesh.visible = arrival ? q >= 0.76 : q < 0.3;
    }
  }
  finish() {
    this.resetMechanicalPose();
