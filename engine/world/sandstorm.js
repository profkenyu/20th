...
export class Sandstorm {
  ...
  update(dt, probe, now) {
    ...
    this.points.material.opacity = 0.062 + (0.5 + 0.5 * Math.sin(now * 23e-5)) * 0.026;
    for (let i = 0; i < this.count; i++) {
      const p = i * 3;
      this.pos[p] += drift;
      this.pos[p + 2] += cross + Math.sin(now * 7e-4 + this.phase[i]) * 3e-3;
      this.pos[p + 1] += Math.sin(now * 13e-4 + this.phase[i]) * 0.16 * dt;
