...
export class GraniteField {
  constructor(heightAt) {
    ...
    const fleck = sin(positionWorld.x.mul(2.17).add(positionWorld.z.mul(2.83))).mul(0.5).add(0.5);
    const quartz = pow(abs(sin(positionWorld.x.mul(5.41).sub(positionWorld.z.mul(3.73)))), 14);
    const mica = pow(abs(sin(positionWorld.x.mul(9.17).add(positionWorld.y.mul(11.3)))), 24);
    const stone = mix(vec3(0.052, 0.058, 0.064), vec3(0.225, 0.204, 0.218), fleck.mul(0.34)).add(vec3(0.2, 0.23, 0.25).mul(quartz.mul(0.25))).add(vec3(0.34, 0.32, 0.38).mul(mica.mul(0.18)));
  ...
  syncTo(x, z, force = false) {
    ...
    for (let dz = -this.radius; dz <= this.radius; dz++) {
      for (let dx = -this.radius; dx <= this.radius; dx++) {
        ...
        this.euler.set(hash2(ix, iz, 8) * 0.24, hash2(ix, iz, 9) * Math.PI * 2, hash2(ix, iz, 10) * 0.2);
