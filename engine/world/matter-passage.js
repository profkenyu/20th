...
const COUNTS = Object.freeze({ high: 4600, mid: 3e3, low: 1700 });
const FALLBACK = Object.freeze({ rover: 8156783, lander: 8751499 });
...
function transferColour(mesh, fallback) {
  ...
  if (colour?.isColor && colour.getHex() !== 16777215) return colour.getHex();
  return fallback;
}
...
function sampleSurface(surface, index, position, colour, offset, fallbackPosition) {
  ...
  if (!surface.triangles.length || surface.total <= 0) {
    ...
    const fallback = new THREE.Color(7829367);
...
export class MatterPassage {
  constructor({ renderer, rover, lander }) {
    ...
    this.compute = Fn(() => {
      ...
      const radial = normalize(vec3(
        source.x.sub(centre.x).add(1e-4),
        source.y.sub(centre.y).mul(0.28).add(1e-4),
        source.z.sub(centre.z).add(1e-4)
      ));
      const phase = seed.x.mul(TAU).add(this.uTime.mul(float(0.42).add(seed.y.mul(0.36))));
      const loosen = smoothstep(0, 1, this.uDissolve);
      const loose = source.add(radial.mul(loosen.mul(float(0.55).add(seed.z.mul(1.2))))).add(vec3(cos(phase), seed.y.sub(0.5), sin(phase)).mul(loosen.mul(0.22)));
      const coilRadius = float(0.035).add(seed.z.mul(0.18)).mul(float(1).sub(this.uCompress));
      const coil = centre.add(vec3(
        cos(phase.add(this.uCompress.mul(8))).mul(coilRadius),
        seed.y.sub(0.5).mul(0.34).mul(float(1).sub(this.uCompress)),
        sin(phase.add(this.uCompress.mul(8))).mul(coilRadius)
      ));
      const collapsed = mix(loose, coil, smoothstep(0, 1, this.uCompress));
      const destination = mix(centre, target, this.uTargetReady);
      const reconstruction = smoothstep(0, 1, this.uReconstruct);
      const residual = vec3(cos(phase), seed.z.sub(0.5), sin(phase)).mul(float(1).sub(reconstruction).mul(0.24));
      const position = mix(collapsed, destination.add(residual), reconstruction);
      const bindEnvelope = float(0.1).add(this.uBind.mul(0.9));
    ...
    const spectral = vec3(0.1, 0.13, 0.16).mul(sin(this.meta.element(vertexIndex).x.mul(TAU)).mul(0.5).add(0.5));
    ...
    this.timeline.add(this.state, { bind: 1, opacity: 1, duration: 1800, ease: "inOut(3)" }, 0).add(this.state, { dissolve: 1, duration: 3600, ease: "inOut(3)" }, 1100).add(this.state, { compress: 1, duration: 3500, ease: "inOut(4)" }, 3600).add(this.state, { reconstruct: 1, duration: 5e3, ease: "inOut(4)" }, 7200).add(this.state, { opacity: 0, duration: 2600, ease: "inOut(2)" }, 12e3);
  }
  _counts() {
    return [
  ...
  captureTarget() {
    ...
    for (const root of Object.values(this.roots)) {
      scales.set(root, root.scale.clone());
  ...
  async update(now = performance.now()) {
    ...
    this.uTime.value = Math.max(0, (now - this.startedAt) / 1e3);
  ...
  snapshot() {
    return {
      ...
      phase: !this.active ? "inactive" : this.state.reconstruct > 0 ? "reconstruct" : this.state.compress > 0 ? "compress" : this.state.dissolve > 0 ? "dissolve" : "bind"
    };
  }
  finish() {
    this.timeline.pause();
