...
const CSS = `
#ti-opening-blueprints{position:fixed;z-index:43;top:var(--frame-top);right:0;bottom:var(--frame-bottom);left:0;
  overflow:hidden;pointer-events:none;visibility:hidden;opacity:0;background-color:#050506;color:#d9dde2;
  transition:opacity .62s cubic-bezier(.4,0,.2,1),visibility 0s linear .62s}
body.ti-blueprints-active #ti-opening-blueprints{visibility:visible;opacity:1;transition-delay:0s;
  transition-duration:.08s,0s;transition-timing-function:linear,linear}
body.ti-blueprints-out #ti-opening-blueprints{visibility:hidden;opacity:0}
body.ti-blueprints-active #ti-prologue{background:transparent}
body.embed #ti-opening-blueprints{display:none}
#ti-opening-blueprints::before{content:"";position:absolute;z-index:2;inset:0;pointer-events:none;opacity:0;
  background-image:
    linear-gradient(rgba(139,255,169,.035) 1px,transparent 1px),
    linear-gradient(90deg,rgba(139,255,169,.035) 1px,transparent 1px),
    linear-gradient(rgba(139,255,169,.06) 1px,transparent 1px),
    linear-gradient(90deg,rgba(139,255,169,.06) 1px,transparent 1px);
  background-size:24px 24px,24px 24px,96px 96px,96px 96px}
#ti-opening-blueprints::after{content:"";position:absolute;z-index:4;inset:0;pointer-events:none;
  opacity:0;background:#050506}
/* There is no held black screen before acquisition: the receiver blinks
   twice, then hands the complete frame directly to the live noise raster. */
body.ti-blueprints-active #ti-opening-blueprints::after{
  animation:bp-entry-flicker .46s steps(1,end) both}
#ti-opening-blueprints .bp-frame{position:absolute;
  top:12px;right:max(14px,calc(var(--safe-right) + 10px));
  bottom:12px;left:max(14px,calc(var(--safe-left) + 10px));
  z-index:1;
  min-width:0;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;
  border:1px solid rgba(217,221,226,.18);background-color:#08090a;
  box-shadow:0 0 0 8px #050506,0 0 0 9px rgba(217,221,226,.055)}
#ti-opening-blueprints .bp-noise{position:absolute;z-index:3;inset:0;display:grid;place-items:center;
  visibility:hidden;opacity:0;background:#050506;transition:opacity .12s linear,visibility 0s linear .12s}
#ti-opening-blueprints .bp-noise canvas{display:block;width:100%;height:100%;image-rendering:pixelated;opacity:.88}
#ti-opening-blueprints .bp-noise::before,#ti-opening-blueprints .bp-noise::after{position:absolute;
  font:600 7px/1.4 'DM Mono',ui-monospace,monospace;letter-spacing:.2em;color:rgba(171,255,191,.52);
  text-transform:uppercase;text-shadow:0 0 8px rgba(76,218,115,.22)}
#ti-opening-blueprints .bp-noise::before{content:'MONITOR / SIGNAL ACQUISITION';top:calc(var(--frame-top) + 18px);left:max(18px,calc(var(--safe-left) + 14px))}
#ti-opening-blueprints .bp-noise::after{content:'NO CARRIER · 00:00:03';right:max(18px,calc(var(--safe-right) + 14px));bottom:calc(var(--frame-bottom) + 18px)}
#ti-opening-blueprints.bp-noise-active .bp-noise{visibility:visible;opacity:1;transition-delay:0s}
#ti-opening-blueprints.bp-noise-active .bp-frame{opacity:0}
/* After acquisition, the static is retained as a low-energy monitor residue.
   It belongs to the instrument, so it sits over both production plates. */
#ti-opening-blueprints.bp-residual-on .bp-noise{visibility:visible;opacity:1;mix-blend-mode:screen}
#ti-opening-blueprints.bp-residual-on .bp-noise canvas{opacity:.13}
#ti-opening-blueprints.bp-residual-on .bp-noise::before,#ti-opening-blueprints.bp-residual-on .bp-noise::after{visibility:hidden}
/* The same raster continues over the written title. It is a transparent
   signal layer rather than a second title card, and dissolves only at entry. */
#ti-opening-blueprints.bp-residual-title{z-index:45;visibility:visible;opacity:1;
  background-color:transparent;transition:opacity 3.6s cubic-bezier(.4,0,.2,1),visibility 0s linear 0s}
#ti-opening-blueprints.bp-residual-title .bp-frame{visibility:hidden;opacity:0}
#ti-opening-blueprints.bp-residual-title .bp-noise{visibility:visible;opacity:1;background:transparent;mix-blend-mode:screen}
#ti-opening-blueprints.bp-residual-title .bp-noise canvas{opacity:.12}
#ti-opening-blueprints.bp-residual-title .bp-noise::before,#ti-opening-blueprints.bp-residual-title .bp-noise::after{visibility:hidden}
body.ti-blueprints-out #ti-opening-blueprints.bp-residual-title{visibility:visible;opacity:1}
body.ti-prologue-out #ti-opening-blueprints.bp-residual-title{opacity:0;visibility:hidden;
  transition:opacity 3.6s cubic-bezier(.4,0,.2,1),visibility 0s linear 3.6s}
#ti-opening-blueprints .bp-head{display:grid;grid-template-columns:1fr auto;align-items:center;gap:16px;
  min-height:42px;padding:0 clamp(12px,1.8vw,25px);border-bottom:1px solid rgba(217,221,226,.13);
  font:700 7px/1.35 'Space Mono',ui-monospace,monospace;letter-spacing:.20em;text-transform:uppercase}
#ti-opening-blueprints .bp-head span:first-child{color:#aeb3b9}
#ti-opening-blueprints .bp-head span:last-child{color:#85f2a8;text-align:right}
#ti-opening-blueprints .bp-work{min-width:0;min-height:0;display:grid;
  grid-template-columns:minmax(0,3.25fr) minmax(220px,1fr)}
#ti-opening-blueprints .bp-drawing{position:relative;min-width:0;min-height:0;overflow:hidden;
  border-right:1px solid rgba(217,221,226,.13);
  background-image:radial-gradient(ellipse at 48% 42%,rgba(82,226,115,.026),transparent 58%);
  background-color:#070809}
#ti-opening-blueprints canvas{display:block;width:100%;height:100%}
#ti-opening-blueprints .bp-view-label{position:absolute;top:12px;left:14px;
  font:500 6px/1.4 'DM Mono',ui-monospace,monospace;letter-spacing:.18em;
  color:#626870;text-transform:uppercase}
#ti-opening-blueprints .bp-spec{min-width:0;min-height:0;overflow:hidden;display:flex;flex-direction:column;
  padding:clamp(14px,2vw,28px);background-color:#0a0b0c}
#ti-opening-blueprints .bp-index{font:700 6px/1.4 'DM Mono',ui-monospace,monospace;
  letter-spacing:.18em;color:#85f2a8;text-transform:uppercase}
#ti-opening-blueprints h2{margin:9px 0 8px;font:700 clamp(13px,1.5vw,20px)/1.15 'Space Mono',ui-monospace,monospace;
  letter-spacing:.06em;color:#d9dde2;text-transform:uppercase}
#ti-opening-blueprints .bp-summary{margin:0 0 clamp(12px,2vh,22px);max-width:36ch;
  font:400 clamp(7px,.62vw,9px)/1.62 'DM Mono',ui-monospace,monospace;
  letter-spacing:.055em;color:#777e87;text-transform:uppercase}
#ti-opening-blueprints dl{margin:0;display:grid;grid-template-columns:1fr;min-height:0}
#ti-opening-blueprints dl div{display:grid;grid-template-columns:minmax(70px,.78fr) minmax(0,1.22fr);
  gap:10px;padding:8px 0;border-top:1px solid rgba(217,221,226,.095)}
#ti-opening-blueprints dt,#ti-opening-blueprints dd{margin:0;font:500 6px/1.38 'DM Mono',ui-monospace,monospace;
  letter-spacing:.10em;text-transform:uppercase}
#ti-opening-blueprints dt{color:#555b63}
#ti-opening-blueprints dd{color:#a8adb3;text-align:right;font-variant-numeric:tabular-nums}
#ti-opening-blueprints .bp-parts{margin-top:auto;padding-top:14px;font:500 6px/1.65 'DM Mono',ui-monospace,monospace;
  letter-spacing:.08em;color:#666d75;text-transform:uppercase}
#ti-opening-blueprints .bp-foot{display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:16px;
  min-height:34px;padding:0 clamp(12px,1.8vw,25px);border-top:1px solid rgba(217,221,226,.13);
  font:500 6px/1.35 'DM Mono',ui-monospace,monospace;letter-spacing:.16em;
  color:#5e646c;text-transform:uppercase}
#ti-opening-blueprints .bp-signal{--dot-size:1.25px;--dot-gap:.75px;--dot-character-gap:2.4px;
  --dot-colour:#85f2a8;--dot-off:.055;--dot-on:.78;--dot-glow:2px}
#ti-opening-blueprints .bp-resolution{color:#858b92;text-align:right}
@keyframes bp-entry-flicker{0%,17%{opacity:1}18%,35%{opacity:0}36%,53%{opacity:1}54%,71%{opacity:0}72%,84%{opacity:1}85%,100%{opacity:0}}
@media(max-width:760px){
  #ti-opening-blueprints .bp-frame{top:9px;right:max(10px,calc(var(--safe-right) + 7px));
    bottom:9px;left:max(10px,calc(var(--safe-left) + 7px));box-shadow:none}
  #ti-opening-blueprints .bp-head{min-height:34px;padding:0 10px;font-size:5.5px;letter-spacing:.12em}
  #ti-opening-blueprints .bp-work{grid-template-columns:minmax(0,2.15fr) minmax(150px,1fr)}
  #ti-opening-blueprints .bp-spec{padding:11px}
  #ti-opening-blueprints h2{margin:6px 0;font-size:clamp(11px,3.1vw,15px)}
  #ti-opening-blueprints .bp-summary{margin-bottom:8px;font-size:5.8px;line-height:1.45}
  #ti-opening-blueprints dl div{grid-template-columns:minmax(56px,.8fr) minmax(0,1.2fr);gap:6px;padding:6px 0}
  #ti-opening-blueprints dt,#ti-opening-blueprints dd{font-size:5.4px;line-height:1.3;letter-spacing:.055em}
  #ti-opening-blueprints .bp-parts{padding-top:8px;font-size:5.2px;line-height:1.48}
  #ti-opening-blueprints .bp-foot{min-height:27px;padding:0 10px;font-size:5.2px;letter-spacing:.09em}
  #ti-opening-blueprints .bp-signal{--dot-size:1px;--dot-gap:.6px;--dot-character-gap:1.8px}
}
@media(prefers-reduced-motion:reduce){
  body.ti-blueprints-active #ti-opening-blueprints::after{animation:none;opacity:0}
}
@media(max-width:600px) and (orientation:portrait){
  #ti-opening-blueprints .bp-work{grid-template-columns:1fr;grid-template-rows:minmax(150px,1fr) auto}
  #ti-opening-blueprints .bp-drawing{border-right:0;border-bottom:1px solid rgba(217,221,226,.13)}
  #ti-opening-blueprints .bp-spec{display:grid;grid-template-columns:minmax(0,.86fr) minmax(0,1.14fr);
    grid-template-rows:auto auto 1fr;column-gap:12px;padding:9px 10px 8px}
  #ti-opening-blueprints .bp-index{grid-column:1;font-size:5.2px}
  #ti-opening-blueprints h2{grid-column:1;margin:4px 0 5px;font-size:11px}
  #ti-opening-blueprints .bp-summary{grid-column:1;grid-row:3;margin:0;font-size:5.2px;line-height:1.42}
  #ti-opening-blueprints dl{grid-column:2;grid-row:1/4;align-self:stretch}
  #ti-opening-blueprints dl div{padding:4px 0}
  #ti-opening-blueprints dt,#ti-opening-blueprints dd{font-size:4.9px}
  #ti-opening-blueprints .bp-parts{display:none}
  #ti-opening-blueprints .bp-foot{grid-template-columns:1fr auto;min-height:24px}
  #ti-opening-blueprints .bp-resolution{display:none}
}
@media(max-height:520px) and (orientation:landscape){
  #ti-opening-blueprints .bp-frame{top:6px;right:max(8px,calc(var(--safe-right) + 5px));
    bottom:6px;left:max(8px,calc(var(--safe-left) + 5px))}
  #ti-opening-blueprints .bp-head{min-height:28px}
  #ti-opening-blueprints .bp-spec{padding:8px 10px}
  #ti-opening-blueprints .bp-summary{display:none}
  #ti-opening-blueprints dl div{padding:4px 0}
  #ti-opening-blueprints .bp-parts{padding-top:5px;font-size:4.8px;line-height:1.35}
  #ti-opening-blueprints .bp-foot{min-height:23px}
}
@media(prefers-reduced-motion:reduce){
  #ti-opening-blueprints{transition-duration:.12s!important}
  #ti-opening-blueprints .bp-noise canvas{opacity:.54}
  #ti-opening-blueprints::before{display:none}
  ...
  body.ti-blueprints-active #ti-opening-blueprints .bp-spec{animation:none!important}
}
`;
...
const ROVER_CALLOUTS = Object.freeze([
  ...
  { name: '6 × 12 SOLAR MATRIX', detail: 'PRIMARY SURFACE POWER' },
...
/**
 * A two-plate opening made from the production machines themselves. Rover
 * edges are resolved from its live meshes; lander edges come from the eight
 * restoration wire fields rebuilt after terrain fitting. The scene camera and
 * mission state are never touched.
 */
export class OpeningBlueprintSequence {
  constructor({ rover, lander, tier = 'mid' }) {
    ...
    /* The monitor must acquire a signal before either machine can be read.
       A fully drawn plate then stays still for three seconds: this is a
       deliberate reading interval, not extra animation. */
    this.timing = this.reduced
      ? { noise: 3500, rover: 1320, roverHold: 3000, gap: 260, lander: 1420, landerHold: 3000 }
      : { noise: 3500, rover: 4000, roverHold: 3000, gap: 500, lander: 4200, landerHold: 3000 };
    ...
    this.el.innerHTML = `
      <div class="bp-frame">
        ...
        <footer class="bp-foot"><span>ACTUAL PRODUCTION GEOMETRY · NOT ILLUSTRATION</span><span class="bp-signal"></span><span class="bp-resolution"></span></footer>
      </div>
  ...
  _apply(elapsed, force = false) {
    ...
    for (const [name, ms] of stages) {
      if (elapsed < offset + ms) {
        current = name; local = elapsed - offset; duration = ms;
        break;
      }
      offset += ms;
    }
    ...
    this.reveal = this.reduced ? 1 : smooth(Math.min(1, local / Math.min(1450, duration * .46)));
  ...
  _present(current) {
    ...
    const rows = rover ? [
      ['Envelope', `${d.x.toFixed(2)} W × ${d.z.toFixed(2)} L × ${d.y.toFixed(2)} H m`],
      ['Contact', '8 wheels / independent stroke'],
      ['Power', '6 × 12 photovoltaic matrix'],
      ['Optics', 'Twin mast / 8 mm sensor'],
      ['Survey', 'Sample arm / lidar cluster'],
      ['Geometry', `${model.meshes} meshes / ${model.segments} lines`],
    ] : [
      ['Envelope', `${d.x.toFixed(2)} W × ${d.z.toFixed(2)} L × ${d.y.toFixed(2)} H m`],
      ['Service stage', 'Ø 6.10 m / faceted pressure hull'],
      ['Load paths', '6 articulated / pad centres Ø 11.10 m'],
      ['Receiving bay', '3.02 W × 4.58 D × 2.82 H m'],
      ['Ramp', '3.14 W × 4.90 L m'],
      ['Restoration', `5 systems / ${model.segments} lines`],
    ];
    this.counter.textContent = rover ? 'PLATE 01 / 02 · ROVER' : 'PLATE 02 / 02 · LANDER';
    ...
    this.summary.textContent = rover
      ? 'Eight terrain contacts, the hinged solar field and the optical mast are resolved from the machine that continues into the mission.'
      : 'Six terrain-fitted load paths and five structural systems are resolved after landing-site placement; no mission state is restored for display.';
    this.metrics.innerHTML = rows.map(([term, value]) =>
      `<div><dt>${term}</dt><dd>${value}</dd></div>`).join('');
    this.parts.textContent = (rover ? ROVER_PARTS : LANDER_PARTS).join(' · ');
    renderDotMatrix(this.signal, rover ? 'RVR/01' : 'LDR/02',
  ...
  _sizeNoise() {
    ...
    if (!rect.width || !rect.height) return;
    /* Coarse raster is intentional: this is a failed monitor acquisition,
       not a high-DPI decorative grain pass. */
  ...
  _drawBlackout(progress) {
    ...
    ctx.fillRect(0, 0, w, h);
    if (progress > .72) {
      ctx.fillStyle = `rgba(${PHOSPHOR_SIGNAL_RGB},${smooth((progress - .72) / .28) * .42})`;
      ctx.fillRect(w * .5, h * .5, 1, 1);
    }
    this.draws++;
  }
  ...
  _drawNoise(progress) {
    ...
    const gain = .34 + Math.min(1, progress) * .28;
    for (let i = 0; i < data.length; i += 4) {
      const staticLevel = Math.floor(Math.random() * 92 * gain);
      const flare = Math.random() > .968 ? 70 : 0;
      data[i] = staticLevel * .34;
      data[i + 1] = staticLevel + flare;
      data[i + 2] = staticLevel * .48;
      data[i + 3] = 255;
    }
  ...
  _drawModel(model, reveal, detail, phase, motion) {
    const { ctx, w, h } = this._prepareCanvas();
    drawGrid(ctx, w, h);
    const pad = Math.max(12, Math.min(24, w * .026));
    const insetGap = Math.max(8, w * .012);
    const insetH = Math.max(54, Math.min(h * .235, 122));
    ...
    drawPlate(ctx, model.views.axon, main, reveal, .78, true);
    drawPlate(ctx, model.views.side, side, detail, .48, false);
    drawPlate(ctx, model.views.top, top, detail, .48, false);
...
function extractRover(rover, limit) {
  ...
  root.traverse(object => {
    ...
    if (!position?.count) { edges.dispose(); return; }
...
function partAnchors(view, partCount) {
  ...
  for (let i = 0; i < view.parts.length; i++) {
    ...
    if (!part) continue;
    const j = i * 4;
    part.x += (view.points[j] + view.points[j + 2]) * .5;
    part.y += (view.points[j + 1] + view.points[j + 3]) * .5;
    part.count++;
  }
  ...
  return sums.map((part, index) => {
    ...
    const y = part.count ? part.y / part.count : view.bounds.minY + height * .5;
    return {
      x,
      y,
...
function drawGrid(ctx, w, h) {
  ...
  for (let x = minor, i = 1; x < w; x += minor, i++) {
    const tick = i % 4 ? 3 : 7;
    ctx.moveTo(x + .5, 0); ctx.lineTo(x + .5, tick);
    ctx.moveTo(x + .5, h); ctx.lineTo(x + .5, h - tick);
  }
  for (let y = minor, i = 1; y < h; y += minor, i++) {
    const tick = i % 4 ? 3 : 7;
    ctx.moveTo(0, y + .5); ctx.lineTo(tick, y + .5);
    ctx.moveTo(w, y + .5); ctx.lineTo(w - tick, y + .5);
  }
  ctx.stroke();
  ctx.restore();
}
...
function drawPlate(ctx, view, rect, reveal, alpha, primary) {
  ...
  const margin = primary ? Math.min(rect.w, rect.h) * .085 : Math.min(rect.w, rect.h) * .12;
  const scale = Math.min((rect.w - margin * 2) / bw, (rect.h - margin * 2) / bh);
  const ox = rect.x + rect.w * .5 - (b.minX + b.maxX) * .5 * scale;
  const oy = rect.y + rect.h * .5 - (b.minY + b.maxY) * .5 * scale;
  const count = view.parts.length;
  const faint = primary ? .055 : .035;
  strokeSegments(ctx, view, count, scale, ox, oy, `rgba(217,221,226,${faint})`, primary ? .72 : .58);
  const visible = Math.max(0, Math.min(count, Math.floor(count * clamp01(reveal))));
  if (!visible) return;
  strokeSegments(ctx, view, visible, scale, ox, oy, `rgba(217,221,226,${alpha})`, primary ? .82 : .68);
  if (primary && reveal < .998) {
    const start = Math.max(0, visible - Math.ceil(count * .035));
...
/* The outbound scan resolves geometry only. On the return journey, each
   physical centroid becomes a retained instrument annotation as the datum
   crosses it from right to left. Labels use fixed edge lanes so the live
   production mesh remains the dominant structure rather than a UI diagram. */
function drawPartCallouts(ctx, view, rect, callouts, motion, phase, reduced) {
  const held = phase >= .995;
  const returning = held || (phase > .5 && motion.direction <= 0);
  if (!returning || !view.anchors?.length) return 0;
  const b = view.bounds;
  const bw = Math.max(1e-6, b.maxX - b.minX);
  const bh = Math.max(1e-6, b.maxY - b.minY);
  const margin = Math.min(rect.w, rect.h) * .085;
  const scale = Math.min((rect.w - margin * 2) / bw, (rect.h - margin * 2) / bh);
  const ox = rect.x + rect.w * .5 - (b.minX + b.maxX) * .5 * scale;
  const oy = rect.y + rect.h * .5 - (b.minY + b.maxY) * .5 * scale;
  const leftCount = Math.ceil(callouts.length / 2);
  const rightCount = Math.floor(callouts.length / 2);
  const inset = Math.max(6, Math.min(10, rect.w * .012));
  const labelWidth = Math.max(72, Math.min(142, rect.w * .31));
  const top = rect.y + Math.max(34, rect.h * .14);
  const bottom = rect.y + rect.h - Math.max(22, rect.h * .09);
  const nameSize = rect.w < 390 ? 4.8 : 5.8;
  const detailSize = rect.w < 390 ? 4.2 : 5.0;
  let visible = 0;
  ...
  for (let i = 0; i < callouts.length; i++) {
    ...
    const alpha = held
      ...
      : reduced
        ? (phase >= .5 ? 1 : 0)
        : smooth(clamp01((scanThreshold - motion.position) / .055));
    if (alpha <= .015) continue;
    visible++;
    const right = i % 2 === 1;
    const lane = Math.floor(i / 2);
    const laneCount = right ? rightCount : leftCount;
    const ly = laneCount <= 1 ? (top + bottom) * .5 : top + (bottom - top) * lane / (laneCount - 1);
    ...
    ctx.fillStyle = `rgba(7,8,9,${.72 * alpha})`;
    ctx.fillRect(right ? labelX - labelWidth - 3 : labelX - 3, ly - 9, labelWidth + 6, 19);
    ctx.strokeStyle = `rgba(${PHOSPHOR_RGB},${.22 + .38 * alpha})`;
    ctx.lineWidth = .7;
    ctx.beginPath();
    ctx.moveTo(ax, ay);
    ctx.lineTo(elbow, ly);
...
/* A cosine transport performs exactly one left→right→left sweep during each
   plate. The second harmonic changes velocity gradually without jitter or a
   software-like constant-rate bounce; reduced motion keeps the datum still. */
function reciprocatingScan(local, duration, reduced) {
  if (reduced) return { position: .5, direction: 0, speed: 0 };
  const t = clamp01(local / Math.max(1, duration));
  const theta = Math.PI * 2 * t + Math.sin(Math.PI * 4 * t) * .035;
  const velocity = Math.sin(theta);
  const speed = Math.abs(velocity);
  return {
    position: .5 - Math.cos(theta) * .5,
    direction: speed < .025 ? 0 : velocity > 0 ? 1 : -1,
    speed,
  };
}
...
function drawReciprocatingScan(ctx, rect, motion, reduced) {
  ...
  ctx.rect(rect.x, rect.y, rect.w, rect.h);
  ctx.clip();
  if (!reduced && motion.direction && motion.speed > .01) {
    ...
    const trailAlpha = .105 * motion.speed;
  ...
  ctx.strokeStyle = `rgba(${PHOSPHOR_RGB},${reduced ? .34 : .48 + motion.speed * .16})`;
  ctx.lineWidth = 1;
