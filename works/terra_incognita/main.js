...
/* ── this world ───────────────────────────────────────────────────────── */
const touchTerminal = typeof navigator !== 'undefined' && navigator.maxTouchPoints > 0
  && (matchMedia('(any-pointer: coarse)').matches || matchMedia('(hover: none)').matches);
/* A hand-held screen is a distinct exhibition surface, not merely a smaller
   desktop. Its reduced backing resolution and phosphor compositor preserve
   the same survey while preventing a tablet from entering the full-colour
   cinematic branch just because its hardware reports desktop-class memory. */
...
const CFG = configure({
  ...
  /* A complete autonomous material passage is longer than four minutes once
     five structural and three reserve holds are included. The gallery reset
     must never erase the work immediately before its final recovery. */
  kiosk: { idleMs: 480000 },
...
const DOCKING_LINES = Object.freeze({
  recall: { r: 0, ko: '구조재 5/5 · 자원 3/3 · 귀환 좌표 압축', en: 'STRUCTURE 5/5 · RESERVES 3/3 · COORDINATE RECALL' },
...
const PLANETS = Object.freeze({
  terra: Object.freeze({
    ...
    lines: LINES, survey: TERRA_SURVEY, next: 'desert', mission: 'loadout',
  }),
...
const COMPLETION_CAPTION = Object.freeze({
  r: 0,
  ko: '착륙선 외부 구성 완료 · 구조재 5종 · 자원 게이지 3계통 충전',
  en: 'LANDER SHELL FIXED · FIVE STRUCTURES · THREE RESERVES CHARGED',
});
...
let prologuePhase = 'blueprints'; // monitor noise → rover → lander → title → release → released
  ...
  if (!e.repeat && (e.code === 'Equal' || e.code === 'NumpadEqual' || e.key === '=')) {
    ...
    if (world === 'terra' && !docking.started
      ...
      if (acquired) {
        ...
        captions.force({
          r: 0,
          ko: '구조재 5종 · 자원 3계통 동시 획득',
          en: 'FIVE STRUCTURES · THREE RESERVES ACQUIRED',
        }, now, 5200);
        kiosk.last = now;
      }
    }
    /* Only PLANET 01 exposes '=' as a public simultaneous-acquisition event.
       Later-planet force controls remain diagnostic so the authored voyage
       cannot be skipped accidentally in the exhibition build. */
    if (!TEST) return;
...
if (disabledSubsystems.length) hud.set('backend', `WebGPU · without ${disabledSubsystems.join(', ')}`, true);
...
window.TI_SEQUENCE = () => ({
  ...
  structure: restoration.structuralCount, reserves: restoration.reserveCount,
...
/* ── releasing the prologue ────────────────────────────────────────────────
   ...
   The title's START control becomes live as the title appears. If it remains
   untouched for eight seconds, the authored route begins by itself; a press
   is the visitor's explicit decision to enter earlier. */
const PROLOGUE_TEXT_MS = 8000;
const ARM_MS = 0;
/* START drains the monitor residue instead of cutting the title away. */
...
function schedulePrologueTimer(timer, delay) {
  ...
  timer.id = setTimeout(() => {
    timer.id = 0; timer.deadline = 0; timer.remaining = null;
    timer.run();
  }, ms);
}
...
window.TI_PROLOGUE = () => ({
  ...
  timers: Object.fromEntries(Object.entries(prologueTimers).map(([name, timer]) => [name, {
    ...
    remaining: timer.remaining ?? (timer.id
      ? Math.max(0, timer.deadline - performance.now()) : null),
  }])),
});
...
function startPrologueSequence() {
  ...
  document.body.classList.remove('ti-prologue-out', 'ti-prologue-reading', 'ti-blueprints-out',
    'ti-entry-blackout', 'ti-entry-revealing');
...
function releasePrologue({ throughBlack = false } = {}) {
  ...
  kiosk.last = now;
  removeEventListener('keydown', onPrologueKey);
  /* The title and its faint monitor noise dissolve together. Only after that
     residual signal is gone does the autonomous route take the whole frame. */
...
const resume = () => {
  ...
  if (completionTableau) restoration?.setCompletionRegistration(
    lander?.setCompletionHighlight(restoration.registrationReduced ? 1
...
/* A slow mobile boot can finish after the document was already backgrounded;
   latch the current state so the opening never starts counting while hidden. */
...
async function frame() {
  ...
  if (world === 'terra' && restoration.complete && !restoration.event
      && lander.parts.every(part => part.state === 'solid')
      && !docking.started && !completionTableau) {
    /* Refresh from this traversal even if a prior session left a valid ledger.
       Persistence is an archive; it is not evidence that the current 5+3 gate
       has physically completed. */
  ...
  if (completionTableau) {
    const tableauProgress = Math.max(0, Math.min(1,
      (now - completionTableau.t0) / COMPLETION_TABLEAU_MS));
    ...
    if (tableauProgress >= 0.70) animeRituals.beginCompletionSilence();
  }
  ...
  const objective = world === 'terra'
    ? (restoration.structureComplete
      ? `RESERVE BANK · ${restoration.reserveCount} / 3`
      : `LANDER SHELL · ${restoration.structuralCount} / 5`)
    : world === 'desert'
  ...
  if (acc >= 320) {
    ...
    hud.set('recon', world === 'terra'
      ? `${restoration.structuralCount}/5 shell · ${restoration.reserveCount}/3 reserve`
      : world === 'desert'
...
async function returnToStart() {
  ...
  /* Capture only after the rover reset, the six terrain-fitted lander load
     paths and all restoration wires are back at PLANET 01. This prevents a
     returning kiosk cycle from exposing geometry left by another planet. */
  if (!location.search.includes('embed')) {
    startPrologueSequence();
  } else {
    openingBlueprints.finish({ preserve: true });
