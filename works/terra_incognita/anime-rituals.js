/**
 * ANIME RITUALS — authored DOM-only thresholds.
 *
 * These clocks never move the camera, rover, lander, or simulation. They make
 * already-existing instrument surfaces cross discrete narrative thresholds:
 * blueprint coordinates bind into the title, a manual START re-locks the
 * receiver, and the completed five-structure/three-reserve register falls
 * quiet before docking.
 */
...
export class AnimeRituals {
  constructor() {
    ...
    this.titleTimeline = createTimeline({ autoplay: false, onUpdate: () => this._renderTitle() })
      .add(this.title, { coordinate: 1, duration: 560, ease: 'out(3)' }, 0)
      .add(this.title, { heading: 1, duration: 640, ease: 'out(3)' }, 130)
      .add(this.title, { rule: 1, duration: 510, ease: 'out(2)' }, 310);
    this.receiverTimeline = createTimeline({ autoplay: false, onUpdate: () => this._renderReceiver(), onComplete: () => {
      this.receiverActive = false;
      this._clearReceiver();
    } })
      .add(this.receiver, { monitor: 1, duration: 1020, ease: 'out(2)' }, 0)
      .add(this.receiver, { instruments: 1, duration: 1320, ease: 'out(3)' }, 430)
      .add(this.receiver, { caption: 1, duration: 760, ease: 'out(2)' }, 1740);
    this.completionTimeline = createTimeline({ autoplay: false, onUpdate: () => this._renderCompletion(), onComplete: () => {
      this.completionActive = false;
      this._finishCompletion();
    } })
      .add(this.completion, { cadence: 1, duration: 105, ease: 'steps(1)' }, 0)
      .add(this.completion, { cadence: 2, duration: 185, ease: 'steps(1)' }, 115)
      .add(this.completion, { cadence: 3, duration: 310, ease: 'steps(1)' }, 310)
      .add(this.completion, { silence: 1, duration: 420, ease: 'inOut(2)' }, 0);
  }
  ...
  reset() {
    ...
    if (this.readout) this.readout.textContent = 'REGISTRATION PASS · WAITING';
  }
  ...
  _renderCompletion() {
    ...
    const interval = this.completion.cadence < 1 ? '120MS'
      : this.completion.cadence < 2 ? '280MS' : '640MS';
    this.readout.textContent = `TELEMETRY HOLD · NEXT SAMPLE ${interval}`;
  }

  _finishCompletion() {
    if (this.readout) this.readout.textContent = 'TELEMETRY HOLD · LINK QUIET';
  }
}
