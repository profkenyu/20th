...
첫 화면에는 영문과 한글 서문, 하단 조작 안내가 표시된다. 12초 뒤 서문이 사라지면 Anime.js 타임라인이 실제 로버 형상을 와이어프레임 설계도에서 완성 모델로 조립한 뒤 자율 탐사를 해제한다. HUD는 시작 8초 뒤 자동으로 사라지며, 화면 왼쪽 가장자리에 포인터를 대거나 `H`를 눌러 다시 표시할 수 있다.
